...
from sketch_model.layers import LayerStructureEmbedding, build_transformer, SketchTransformer, RetNet
...
class SketchLayerClassifierModel(nn.Module):
    def __init__(
        ...
        self.retnet = RetNet(12, self.hidden_dim, 2048, 8, double_v_dim=True).to('cuda')
    ...
    def forward(
        ...
        batch_msg = NestedTensor(x[:, :, -42:-10].int(), mask)
        batch_name = NestedTensor(x[:, :, -74:-42].int(), mask)
        ...
        if self.config.use_image:
            x, pos_embed, image_embeds = self.structure_embed(
                batch_img, batch_name, batch_bbox, batch_color, batch_class, batch_msg)
        else:
