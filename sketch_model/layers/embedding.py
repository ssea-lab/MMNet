...
class LayerStructureEmbedding(nn.Module):
    def __init__(
        ...
        self.msg_embeder: Optional[nn.Module] = None  # Message
        self.init_msg_embedding()
    ...
    def init_msg_embedding(self):
        if self.config.use_msg:
            self.msg_embeder = TextEmbedding(
                self.embedding_dim,
                self.config.vocab_size,
                self.config.pad_token_id,
                self.config.dropout,
    ...
    def forward(
        ...
        if self.config.use_msg:
            embeds.append(self.msg_embeder(messages.tensors))
        if self.config.use_color:
