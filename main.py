...
def main(config: SketchModelConfig):
    ...
    dataset_train = build_dataset(config.train_index_json,
                                  Path(
                                      config.train_index_json).parent.__str__(),
                                  tokenizer,
                                  cache_dir=config.cache_dir,
                                  use_cache=config.use_cache,
                                  remove_text=config.remove_text)
...
def train_one_epoch(
    ...
    print_freq = 10
    y_pred, y_true = [], []
    for (batch_img, batch_name, batch_msg, batch_bbox, batch_color,
        ...
        batch_msg: NestedTensor = batch_msg.to(device)  # [8, 176, 32]
        ...
        outputs = model(
            flatten_input_for_model(batch_img, batch_name, batch_msg, batch_bbox,
                                    batch_color,
